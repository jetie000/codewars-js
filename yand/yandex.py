...
def metrika_wrapper(callback: Callable[..., None]) -> Callable[..., dict[str, Any]]:
    ...
    def wrapper(user: dict[str, Any]) -> list[dict[str, Any]]:
        if user and user['login'] and user['password'] and user['date']:
            currStatDay = [i for i,x in enumerate(statDays) if x['date']==user['date'].strftime("%Y-%m-%d")]
            isUser = [i for i,x in enumerate(users) if x['date']==user['date'].strftime("%Y-%m-%d") and x['login'] == user['login'] and x['password'] == user['password']]
            if currStatDay:
                if not isUser:
                    users.append(user)
                ...
                statDays.append(dict({ 'date': user['date'].strftime("%Y-%m-%d"), 'users': 1 }))
                if not isUser:
                    users.append(user)
                callback()
        return statDays
